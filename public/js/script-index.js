"use strict";
var nrOfMovies = $('.block_holder  li').length;
var learnRate = "0,7";
var discardRate = "0,5";
var maxChoices = 10;
...
*/  
  ...
  // Look for trailer when hovering over movie
  $('.block_holder li').click(function() {
  ...
  $('.block_holder li .select').click(function() {
    // Find which movie was clicked
    var moviePos = $(this).parent().index();
...
function loadMovieInfo(itemNr, mID, mType) {
  ...
  return getMovieInfo(mID, mType, function(movieInfo) {
    ...
    $('.block_holder li:nth-child(' + itemNr + ') .cover').css('background', 'url(' + pictureURL + ')');
...
function resetMovies() {
  ...
  for(var i=1; i<=nrOfMovies; i++) {
    ...
    $('.block_holder li:nth-child(' + i + ') .cover').css('background-image', 'url(' + pictureURL + ')');
    $('.block_holder li:nth-child(' + i + ') .movietitle').text('{title}');
    $('.block_holder li:nth-child(' + i + ') .movieyear').text('{year}');
  }
}
